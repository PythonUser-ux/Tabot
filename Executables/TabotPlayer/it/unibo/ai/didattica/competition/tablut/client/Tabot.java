...
public class Tabot extends TablutClient {
	...
	public int WhiteCapture(String state, int[] destT, List<int[]> camp_cells){        // DA OTTIMIZZARE
		...
		return score;}
	public int BlackCapture(String state, int[] destT, List<int[]> camp_cells){        // DA OTTIMIZZARE
		int[] dest = destT.clone();
		int temp=dest[0];
		dest[0]=dest[1];
		dest[1]=temp;                                      // PERCHE' LE COORDINATE DELLE CASELLE DI DESTINAZIONE SONO SWAPPATE
		...
		if(dest[0]<7){                                  // Se c'è spazio per una cattura a destra
			...
			cou1[1]=indexf/10;
			second_cell=state.substring(indexf+2,indexf+3);
			if ((state.substring(indexf+1,indexf+2).equals("W") || state.substring(indexf+1,indexf+2).equals("K")) && !in(camp_cells,cou1) && (second_cell.equals("T") || second_cell.equals("B") || in(camp_cells,cou2))){score++;}
		}
		if(1<dest[0]){                                  // Se c'è spazio per una cattura a sinistra
			cou2[0]=indexf%10-2;
			cou2[1]=indexf/10;
			...
			cou1[1]=indexf/10;
			second_cell=state.substring(indexf-2,indexf-1);
			if ((state.substring(indexf-1,indexf).equals("W") || state.substring(indexf-1,indexf).equals("K")) && !in(camp_cells,cou1) && (second_cell.equals("T") || second_cell.equals("B") || in(camp_cells,cou2))){score++;}
		}
		if(dest[1]<7){                                  // Se c'è spazio per una cattura in basso
			cou2[0]=indexf%10;
			cou2[1]=indexf/10+2;
			...
			cou1[1]=indexf/10+1;
			second_cell=state.substring(indexf+20,indexf+21);
			if ((state.substring(indexf+10,indexf+11).equals("W") || state.substring(indexf+10,indexf+11).equals("K")) && !in(camp_cells,cou1) && (second_cell.equals("T") || second_cell.equals("B") || in(camp_cells,cou2))){score++;}
		}
		if(1<dest[1]){                                  // Se c'è spazio per una cattura in alto
			cou2[0]=indexf%10;
			cou2[1]=indexf/10-2;
			...
			cou1[1]=indexf/10-1;
			second_cell=state.substring(indexf-20,indexf-19);
			if ((state.substring(indexf-10,indexf-9).equals("W") || state.substring(indexf-10,indexf-9).equals("K")) && !in(camp_cells,cou1) && (second_cell.equals("T") || second_cell.equals("B") || in(camp_cells,cou2))){score++;}
		}
		return score;}
	...
	public int CanCapture(String state, List<int[]> camp_cells, int color){        // DA OTTIMIZZARE   // 0 è il bianco
		int[] cou = new int[2];
		int[] cou1 = new int[2];     // The swapped current couple
		if(color==0){
		outerloop:{
		for(int i=0;i<9;i++){
			for(int j=0;j<9;j++){
				int index=i+10*j;
				if(state.substring(index, index+1).equals("W") || state.substring(index, index+1).equals("K")){
					for(int pos=i+1;pos<9;pos++){cou[0]=pos; cou[1]=j; cou1[0]=cou[1]; cou1[1]=cou[0];if(state.substring(cou[0]+cou[1]*10,cou[0]+cou[1]*10+1).equals("O") && !in(camp_cells, cou) && WhiteCapture(state, cou1, camp_cells)!=0){return 1;}else{break outerloop;}}
					for(int pos=i-1;pos>-1;pos--){cou[0]=pos; cou[1]=j; cou1[0]=cou[1]; cou1[1]=cou[0];if(state.substring(cou[0]+cou[1]*10,cou[0]+cou[1]*10+1).equals("O") && !in(camp_cells, cou) && WhiteCapture(state, cou1, camp_cells)!=0){return 1;}else{break outerloop;}}
					for(int pos=j+1;pos<9;pos++){cou[0]=i; cou[1]=pos; cou1[0]=cou[1]; cou1[1]=cou[0];if(state.substring(cou[0]+cou[1]*10,cou[0]+cou[1]*10+1).equals("O") && !in(camp_cells, cou) && WhiteCapture(state, cou1, camp_cells)!=0){return 1;}else{break outerloop;}}
					for(int pos=j-1;pos>-1;pos--){cou[0]=i; cou[1]=pos; cou1[0]=cou[1]; cou1[1]=cou[0];if(state.substring(cou[0]+cou[1]*10,cou[0]+cou[1]*10+1).equals("O") && !in(camp_cells, cou) && WhiteCapture(state, cou1, camp_cells)!=0){return 1;}else{break outerloop;}}
				}}}}}
		else{
			outerloop:{
			for(int i=0;i<9;i++){
				for(int j=0;j<9;j++){
					int index=i+10*j;
					if(state.substring(index, index+1).equals("B")){
						for(int pos=i+1;pos<9;pos++){cou[0]=pos; cou[1]=j; cou1[0]=cou[1]; cou1[1]=cou[0];if(state.substring(cou[0]+cou[1]*10,cou[0]+cou[1]*10+1).equals("O") && !in(camp_cells, cou) && BlackCapture(state, cou1, camp_cells)!=0){return 1;}else{break outerloop;}}
						for(int pos=i-1;pos>-1;pos--){cou[0]=pos; cou[1]=j; cou1[0]=cou[1]; cou1[1]=cou[0];if(state.substring(cou[0]+cou[1]*10,cou[0]+cou[1]*10+1).equals("O") && !in(camp_cells, cou) && BlackCapture(state, cou1, camp_cells)!=0){return 1;}else{break outerloop;}}
						for(int pos=j+1;pos<9;pos++){cou[0]=i; cou[1]=pos; cou1[0]=cou[1]; cou1[1]=cou[0];if(state.substring(cou[0]+cou[1]*10,cou[0]+cou[1]*10+1).equals("O") && !in(camp_cells, cou) && BlackCapture(state, cou1, camp_cells)!=0){return 1;}else{break outerloop;}}
						for(int pos=j-1;pos>-1;pos--){cou[0]=i; cou[1]=pos; cou1[0]=cou[1]; cou1[1]=cou[0];if(state.substring(cou[0]+cou[1]*10,cou[0]+cou[1]*10+1).equals("O") && !in(camp_cells, cou) && BlackCapture(state, cou1, camp_cells)!=0){return 1;}else{break outerloop;}}
					}}}}}
		return 0;
	}
	...
	public void run() {
		...
		List<Integer> values_fifth = new ArrayList<Integer>();
		int[] buf;
		for(int i = 0; i < 17; i++){
			buf = new int[2];
			buf[0]=x[i];
